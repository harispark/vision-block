...
{
    public class ViewModelBlock_ContrasteNdg : ViewModel_EmptyBlock
    {
        [DllImport("ImageProcessing.dll", CallingConvention = CallingConvention.Cdecl)]
        private static extern unsafe void ContrasteNdg(IntPtr data, int stride, int nbLig, int nbCol, double contraste);

        public static Bitmap ContrasteNdg(Bitmap bmp, double contraste)
        {
            unsafe
            {
                ...
                ContrasteNdg(bmpData.Scan0, bmpData.Stride, bmp.Height, bmp.Width, contraste);
                bmp.UnlockBits(bmpData);
            }
            return bmp;
        }

        public ViewModelBlock_ContrasteNdg()
        {
        ...
        public override string DescriptionButton { get { return "Augmentation des contrastes d'une image NDG"; } }
        public override string NameButton { get { return NAME; } }
        public override Type xamlBlockType { get { return typeof(Block_ContrasteNdg); } }


        public override void ExecuteTraitementBlock(List<Bitmap> imgs)
        {
            ImageToProcess = ContrasteNdg(CloneBitmapThreadSafe(imgs.First()), ContrasteValue);
        }
