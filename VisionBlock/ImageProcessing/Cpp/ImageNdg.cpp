...
CImageNdg::CImageNdg(int hauteur, int largeur, int valeur) {
	...
	//choixPalette("grise"); // palette grise par défaut, choix utilisateur  		//todo réimplementer ceci
	if (valeur != -1) 
			...
			if (header.bfType != MAGIC_NUMBER_BMP) 
				...
				if (infoHeader.biCompression > 0) 
					...
					if (infoHeader.biBitCount == 8) {
						...
						this->m_sNom.assign(name.begin(),name.end()-4);
						this->m_pucPalette = new unsigned char[256*4];	
						//this->choixPalette("grise"); // palette grise par défaut 		//todo réimplementer ceci
						this->m_pucPixel = new unsigned char[infoHeader.biHeight * infoHeader.biWidth];
...
std::vector<int> CImageNdg::histogrammeInt(bool enregistrementCSV, int pas) {
...
CImageNdg CImageNdg::transformation(const std::string& methode,int vMinOut, int vMaxOut) {
	...
	//out.choixPalette(this->lirePalette()); // conservation de la palette 		//todo réimplementer ceci
...
CImageNdg CImageNdg::Contraste(double contrastValue, byte brightness)
{
		CImageNdg out = CImageNdg(this->lireHauteur(), this->lireLargeur());

		for (int i = 0; i < this->lireNbPixels(); i++)
		{
			byte pixel = this->operator()(i);
			double int1 = (double)pixel - 128.0;
			double int2 = contrastValue * int1;
			double int3 = int2 + 128.0 + (double)brightness;
